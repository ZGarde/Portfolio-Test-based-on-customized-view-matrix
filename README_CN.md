...
本项目是一个面向研究、课程展示和策略原型验证的多资产组合优化框架。它围绕五类核心资产展开：黄金、美国国债、比特币、原油和纳斯达克风险资产。项目从数据获取、收益率构建、因子工程、因子质量审查，到 Black-Litterman 静态组合构建，再到 rolling out-of-sample 回测，形成一条完整的量化研究流水线。
...
- 金融工程、量化投资、数据科学课程项目；
- 多资产组合优化研究展示；
- Black-Litterman 与因子研究原型；
- 面试作品集中的研究型项目。

它不适合直接作为实盘交易系统。原因是数据源、交易成本、滑点、期货连续合约、保证金、税费、盘中价格和执行细节都仍有简化。
...
|---|---|---|---|---|
| 黄金中美时差因子 | `gold_china_to_us_factor` | 中国黄金夜盘可能领先美国黄金交易时段 | Gold | 数据源是 SGE proxy，非确认 SHFE AU；日频无法完全刻画盘中关系 |
